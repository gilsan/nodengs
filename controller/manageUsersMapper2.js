...
const insertHandler = async (req) => {  
    ...
    try {
        const request = pool.request()
          ...
          .input('password', mssql.VarChar, encrypted) 
...
const updateHandler = async (req) => { 
    ...
    logger.info('[107][manageUser updte]user_id=' + user_id + ", " + password +"," + encrypted + ", " + user_nm + ", " + user_gb 
...
exports.listUsers = (req, res, next) => {  
    ...
    result.then(data => { 
        ...
        ? data.map(u => ({
            ...u,
            password: u.password ? decrypt(u.password.trim()) : ''
            }))
        : {
            ...data,
            password: data.password ? decrypt(data.password.trim()) : ''
            };
