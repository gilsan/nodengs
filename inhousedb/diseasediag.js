...
var tsvData = '../inhouseupload/diseasediag.txt';
var rowCount = 0;
...
rowData.forEach ( async (row, index) =>  {
  ...
  if (rowCount >= 0) {
    ...
    logger.info('length =' + row.length);
      ...
      var test_code = nvl(row[3].replace( /"/gi, ''), "");
      ...
      if (test_code != "") {
        ...
        logger.info('[84][diseasediag]disease=' + temp_data);
        
        temp_data = nvl(row[2].replace( /"/gi, ''), "");
        type = findChar(temp_data);   

        if (disease != "") {
 
          logger.info('[87][diseasediag]gene=' + disease);
          
            const sql =`insert_diseasediag` ;
