...
async function delData() {
  ...
  logger.info('[77][benign]sql=' + sql2);
  ...
  result2.then(data => {
    ...
    logger.error('[97][benign]del err=' + err.message);
  } 
}

async function processData() {
  var tsvData = '../inhouseupload/benign.txt';
  var rowCount = 0;
    ...
    if (rowCount >= 0) {
        ...
        try {
          ...
          await request.execute(sql, (err, recordset, returnValue) => {
              if (err)
              {
                console.log ("172[benign]err message=" + err.message);
              }

              console.log("[175][benign]recordset="+ recordset);
              console.log("[176][benign]returnValue="+ returnValue);
          ...
          logger.error('[185][benign]err=' + err.message);
      }  
    } 
  });// end of if loop
}
