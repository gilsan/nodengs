...
const insertOR = async (originalname, dirPath, pathology_num) => {
  await poolConnect;
  logger.info('[32][researchupload]insertOR]originalname=' + originalname + ", dirPath=" + dirPath
  ...
  logger.info('[36][researchupload][INSERT OR SQL=' + sql) ;
  try {
    ...
    logger.error('[46][researchupload][insertOR]err=' + error.message);
  }   
}
...
const updateORpath = async (filename, orpath, pathology_num) => {
   ...
   logger.info('[53][researchupload]update patient path]filename=' + filename
   ...
   logger.info('[57][researchupload][updateORpath]sql=' + sql);
   ...
   try {
    ...
    logger.error('[66][researchupload][updateORpath]err=' + error.message)
   }
}
...
const updateIRpath = async (filename, irpath, pathology_num) => {
  ...
  logger.info('[73][researchupload]update IR path]filename=' + filename
                     + ", dirPath=" + dirPath + ", pathology_num=" + pathology_num); 

  const sql ="update patientinfo_path \
                        set tsvirfilename = @filename, \
                        irpath = @irpath , \
                        screenstatus = '0' \
                        where pathology_num=@pathology_num";
  logger.info('[73][researchupload]update IR path]sql=' + sql);
  try {
   const request = pool.request()
     .input('filename', mssql.VarChar, filename)
     .input('pathology_num', mssql.VarChar, pathology_num)
   ...
   const result = await request.query(sql);
    ...
    logger.error('[86][researchupload][updateIRpath]err='+ error.message);
  }
}

const updatePatient = async (pathology_num) => {

  logger.info('[92][researchupload]update patient path]pathology_num=' + pathology_num);
  await poolConnect;
  const sql ="update patientinfo_path \
                set screenstatus = '0'  \
              where pathology_num=@pathology_num";
  logger.info('[97][researchupload][updatePatient]sql=' + sql);
  try {
    const request = pool.request()
      .input('pathology_num', mssql.VarChar, pathology_num);
    ...
    logger.error('[103][researchupload][updatePatient]err=' + error.message);
  }
}

const insertIR = async (originalname, dirPath, pathology_num) => {
  await poolConnect;
  logger.info('[112][researchupload]insert pathTsv]originalname=' + originalname
                    + ", dirPath=" + dirPath + ", pathology_num=" + pathology_num); 

  const sql = "insert into pathTsvUpload (filename, path, pathology_num) "
    + "values (@originalname, @dirPath, @pathology_num)";
  logger.info('[73][researchupload][insert pathTsv]sql=' + sql); 
    ...
    try {
      ...
      logger.info('[73][researchupload][insertIR]err=' + error.message);
    }   
}
...
var storage = multer.diskStorage({
    destination: function (req, file, cb) {
      ...
      if (day < 10) {
	    ...
	    if (month < 10) {             
		    if (day < 10) {
			    dirPath = 'research/'+ year + '/' + thisMonth + '/' + thisDay;
		    } else {
          dirPath = 'research/'+ year + '/' + thisMonth + '/' + day;
		    }
	    } else {
  		   if (day < 10) {
			        dirPath = 'research/'+ year + '/' + month + '/' + thisDay;
		     } else {
			        dirPath = 'research/'+ year + '/' + month + '/' + day;
		     }
      }
    ...
    upload(req, res, function (err) {
      ...
      for(let item of req.files) {
        ...
        exceluploadedFiles.push({filename: item.originalname});	 
          ...
          console.log(' researchupload [198][검체번호] ' + pathologyNum + '  [타입] ' + type + ' [화일명] ' + filename + '[화일타입]' + fileType);
          logger.info(' researchupload [198][검체번호] ' + pathologyNum + '  [타입] ' + type + ' [화일명] ' + filename + '[화일타입]' + fileType);
          ...
          if (month1 < 10) {
            ...
            if (day1 < 10) {
              ...
              dirPath2 = 'research_temp/' + year1 + '/' + thismonth + '/' + thisday;
            } else {
              today = year1 + '-' + thismonth + '-' + day1;
              dirPath2 = 'research_temp/' + year1 + '/' + thismonth + '/' + day1;
            }
          } else {
            if (day1 < 10) {
              today = year1 + '-' + month1 + '-' + thisday;
              dirPath2 = 'research_temp/' + year1 + '/' + month1 + '/' + thisday;
            } else {
              today = year1 + '-' + month1 + '-' + day1;
              dirPath2 = 'research_temp/' + year1 + '/' + month1 + '/' + day1;
            }       
          }
          ...
          // destinat`ion will be created or overwritten by default.
        ...
        */
          ...
          logger.info('[290][researchupload]'
                                  + ' [검체번호] ' + pathologyNum + '  [경로] ' + dirPath
          ...
          if (type === 'N') {  // 신규 입력
            ...
            if (fileType === 'IR') {
              ...
              .catch(error => {
                logger.error('[73][researchupload][update IR Path]err=' + error.message);
              });
              ...
              irResult.then(data => {
                ...
                logger.error('[282][researchupload][insert IR]err='  + error.message);
              });
              ...
              irPathUpdate.then(data => {
                ...
                logger.error('[289][researchupload][update or path]err=' + error.message);
              });
              ...
              orResult.then(data => {
                ...
                logger.error('[296][researchupload][insert OR]err=' + error.message);
              });
          ...
          if (fileType === 'RNA') {
            ...
            irPathUpdate.then(data => {
              // res.json({message: ' OR 추가 했습니다.'});
                ...
                logger.info('[313][researchupload][update IR path]err=' + error.message);
              });
              ...
              orResult.then(data => {
                ...
                logger.error('[320][researchupload][insert OR]err=' + error.message);
              });      
            ...
            irPathUpdate.then(data => {
              ...
              logger.error('[330][researchupload][update IR path]err=' + error.message);
            });
            ...
            irResult.then(data => {
              ...
              logger.error('[338][researchupload][insert IR]err=' + error.message);
            });
