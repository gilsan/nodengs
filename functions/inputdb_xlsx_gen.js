...
const  messageHandler_del = async (testedID) => {
    ...
    logger.info("[17][inputdb_gen del][sql_del]" + sql_del);
    logger.info("[17][inputdb_gen del][testedId]" + testedID);
...
const  messageHandler = async (
  ...
  await poolConnect; // ensures that the pool has been created
    ...
    logger.info ('[56][inputdb_gen]genes=' + genes + ', exon=' + exon  + ', functional_impact=' + functional_impact);
    logger.info ('[56][inputdb_gen]transcript=' + transcript + ', coding=' + coding + ', amino_acid_change=' + amino_acid_change );
    logger.info ('[56][inputdb_gen]GnomAD_AF=' + GnomAD_AF + 'zygosity=' + zygosity );
    logger.info ('[56][inputdb_gen]testedID=' + testedID );
